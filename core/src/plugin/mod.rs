// Phase 6 タスク4で host/allowlist.rs へ移設(旧パス互換。削除は本 Phase の Task 5)。
pub use crate::host::allowlist;
// Phase 6 タスク4で settings/filesystem.rs へ移設(旧パス互換。削除は本 Phase の Task 5)。
pub use crate::settings::filesystem;
...
// Phase 6 タスク4で registry/select_options.rs へ移設(旧パス互換。削除は本 Phase の Task 5)。
pub(crate) use crate::registry::select_options;
// Phase 3 で settings/store.rs へ移動(旧パス互換。削除は Phase 6)。
pub use crate::settings::store as settings;
// Phase 6 タスク4で settings/sidecar.rs へ移設(旧パス互換。削除は本 Phase の Task 5)。
pub use crate::settings::sidecar;
