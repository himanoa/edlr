    ...
    for capability in capabilities.iter_mut() {
        match capability {
            CapabilityRequest::Http { hosts, reason } => {
                ...
                crate::capability::validate::reject_invisible_chars("reason", &trimmed)
                    .map_err(ManifestError::BadCapability)?;
                *reason = trimmed;

                for host in hosts.iter() {
                    crate::capability::validate::reject_invisible_chars("host", host)
                        .map_err(ManifestError::BadCapability)?;
                    crate::capability::validate::validate_host(host)
                        .map_err(ManifestError::BadCapability)?;
                }
            }
        }
...
pub(crate) fn validate_sidecars(sidecars: &mut [SidecarRequest]) -> Result<(), ManifestError> {
    ...
    for sidecar in sidecars.iter_mut() {
        ...
        crate::capability::validate::reject_invisible_chars("reason", &trimmed)
            .map_err(ManifestError::BadSidecar)?;
        sidecar.reason = trimmed;

        for arg in &sidecar.args {
            crate::capability::validate::reject_invisible_chars("args", arg)
                .map_err(ManifestError::BadSidecar)?;
        }
    }
    Ok(())
...
pub(crate) fn validate_filesystem(requests: &mut [FilesystemRequest]) -> Result<(), ManifestError> {
    ...
    for request in requests.iter_mut() {
        ...
        crate::capability::validate::reject_invisible_chars("reason", &trimmed)
            .map_err(ManifestError::BadFilesystem)?;
