...
- 保存時の検証:
  ...
  - **edlr 自身の状態ディレクトリ(settings / grants / plugins)、およびその祖先・配下で
    ないこと** — ここを read-write で承認できると、ファイルアクセスの承認が他 capability への
    昇格経路になる(`<grants-dir>/<other-id>.json` で他プラグインの承認を捏造、
    `<plugins-dir>/<id>/plugin.wasm` の差し替えで承認済みプラグインの中身をすり替え、
    `<settings-dir>/<id>.sidecar.json` の `command` の書き換え)。祖先も拒否するのが要点で、
    「そのもの」しか見ないと `~/.config` を選ばれて素通りする。実パスは
    `core/src/bin/edlr.rs` から注入する(CLI で上書きされた場所を使う)
- 検証に失敗した場合は何も書き込まない
...
### この 3 段が保証しないこと(ハードリンク・バインドマウント)

3 段の検証が拘束するのは**パス解決**であって、ルート内のディレクトリエントリが最終的に
どの inode を指しているかではない。したがって:

- **既存のハードリンク**: `driver-fs` に `link` 操作は無く、`write` は tmp + `rename` なので、
  **wasm プラグインがルート内にハードリンクを作る経路は存在しない**。しかし**外部の主体が
  事前に張ったハードリンク**が承認したフォルダ内にあれば、`read` / `append` はその内容
  (ルート外の inode)に到達しうる。`openat2` はパス解決の拘束なので、これを検出できない
- **バインドマウント**: 同じ理由で、承認したフォルダ内に既にあるバインドマウントは
  `RESOLVE_BENEATH` では止まらない

いずれも「承認したフォルダの中に、外部の主体が事前に仕掛けてある」場合に限られる。
つまり保証の正確な形は「**プラグインがルートの外へ出る経路を作れない**」であって、
「ルート内のどのエントリも必ずルート内の実体を指す」ではない。
...
- `list` は `prefix` 配下を**再帰的に**列挙し、**ファイルのみ**を返す(ディレクトリ自体は
  含めない)。返すエントリ数の上限は 10,000 件で、超えたら `too-large`(呼び出し期限を
  食い潰さないため)。加えて、**走査したディレクトリエントリの総数**にも別途 100,000 件の
  予算がある — 返却数の上限だけでは、ディレクトリや FIFO しか含まない巨大ツリー
  (プラグイン自身が `write` を繰り返して作れる)が上限に一度も触れずに全走査されてしまう。
  この予算は dirent を**読みながら**効く(1 ディレクトリを読み終えてから数えるのでは、
  単一ディレクトリに大量のエントリを置かれたときに効かない)
- `read` / `stat` / `append` は**通常ファイルのみ**を受け付ける(`invalid-path`)。
  FIFO に対する `open` は相手が現れるまで返らず、ホスト呼び出し中はエポック割り込みが
  効かないため、プラグイン専用スレッドが恒久的に固まる。`write` は `O_CREAT | O_EXCL` の
  tmp を経由するので既存の FIFO を開くことはない
- `list` が返す名前は、必ず他の操作にそのまま渡せる。1 段目の構文検証を通らない名前
