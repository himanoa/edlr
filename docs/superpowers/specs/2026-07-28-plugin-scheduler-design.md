...
- ~~打ち漏らし(デーモン停止中に過ぎた定刻)の追い掛け実行~~ --
  当初はスコープ外としたが、後から `catch-up = true` のオプトインとして
  実装した(`docs/plugins.md` の「打ち漏らしの追い掛け実行」参照)
...
- 「時刻は単調時計(`Instant`)ベースで管理し、cron の定刻計算にだけ壁時計を
  使う」という上記の方針は、当初の実装では見送って interval・cron の両方を
  `chrono::Local` の壁時計で統一していた(時刻表現をひとつに揃えるほうが
  実装・テストが単純で、`plugins/list` の `next` フィールドはどのみち壁時計へ
  変換が要るため)。しかし NTP のステップ補正やサスペンド/レジュームで
  interval の発火が任意に遅延・合体する影響が実用上無視できないため、
  **設計どおりの方針に戻した**。
- 現在は `core/src/plugin/schedule.rs` の `Clock`(壁時計 + 単調時計のペア)で
  現在時刻を受け取り、interval は `Instant`、cron は `chrono::Local` で追う。
  `plugins/list` の `next`(壁時計の ISO8601)は `Clock` を基準に単調時刻を
  壁時計へ変換して組み立てる(詳細は `schedule.rs` のモジュールドキュメント参照)。
