...
- `command` が未設定のサイドカーは承認できない。この検証は `Registry::set_sidecar_grant` 自身が
  強制する(UI の承認トグルがパス入力後にしか有効化されないのは、これと同じ規則を UI 側でも
  先回りして見せているだけで、唯一の防衛線ではない -- RPC を直接叩いた場合も拒否される)
...
**承認画面での開示(最終レビューで見つかった Important な取りこぼしの修正)**: 承認対象の `port`
はマニフェストの既定値であり、ユーザーが Plugins UI で変更できる。承認は「サイドカーを 1 つも
起動していなくても」この暗黙許可を確定させる -- プラグインは承認された瞬間から
`http://127.0.0.1:<port>` への通信を試みられる。承認しようとしているポートで既に他のプログラム
(別プラグインのサイドカーやローカルの開発サーバなど)が listen していれば、そのプログラムとも
通信できてしまう。`validate_no_overlap`(`sidecar.rs`)が防ぐのはあくまで**同一プラグイン内**の
ポート範囲重複であり、別プラグインや edlr 管理外のプロセスとの重複までは検出しない。この帰結を
ユーザーが承認前に把握できるよう、Plugins UI の承認画面(`SidecarSection.tsx`)には実際に許可され
るポート一覧とネットワークの帰結を明記してある(承認プログラムを実行できる、という文言だけでは
不十分だった)。
...
**「デーモン停止」の実装(最終レビューで見つかった Critical な取りこぼしの修正)**:
`edlr` バイナリの `main` は `tokio::signal::unix::signal` で SIGTERM/SIGINT を捕捉し、受け取ったら
明示的な shutdown シーケンス(`registry.stop_all_sidecars()`)を経て終了する。これが無いと、
デーモンにシグナルハンドラが一切無いままシグナルのデフォルト動作(即死)で終了し、
サイドカーが後始末される機会を得られない。Tauri デスクトップアプリ側の `stop_child`
(デーモンの停止を担う唯一の箇所)も、以前は `Child::kill()`(SIGKILL)で無条件に即殺していたが、
SIGTERM を送って猶予を与えてから(応答が無ければ)SIGKILL にフォールバックする形へ直した。
