...
しない。実行時には `unknown-driver` / `unknown-topic` を返す。
...
`PLUGIN_EVENT_QUEUE_CAPACITY`(32)とは**別の定数**(既定 64)を持たせる。
...
- **`publish`(プラグイン → ドライバ)は満杯なら捨てずに `queue-full` を返す。** publish は結果を
  ...
  作者が気づけない
- **`emit`(ドライバ → プラグイン)の配信は、既存のイベント配信と同じく古いものから捨てる。**
  遅いプラグイン 1 個がドライバ全体を止める事態を避けるため
...
- **デーモン終了時の後始末に効く。** ブロッキング HTTP 中のドライバは停止要求に即応できず、
  ...
  `STOP_GRACE`(65 秒)はサイドカー停止(3 秒 × 20 インスタンス)を想定した値なので、ドライバの
  期限が加わると足りなくなり得る。**既存に倣い、この関係もコンパイル時アサーションで固定する**
