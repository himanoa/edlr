...
| `delivery` | | 作業キュー満杯時の扱い。`"lossy"`(既定): 最も古いイベント/バス配信を捨てて新着を積む(読み上げ通知など「最新が大事」な用途向け)。`"reliable"`: 無制限キューで一切捨てない(アップロードや集計など取りこぼし厳禁な用途向け。プラグインが長時間詰まるとメモリに溜まり続けるので必要なものだけ宣言する) |
...
- **`plugins/list`**(`params` 不要) — `plugins-dir` のパスと、ロード済み全プラグインの
  ...
  意味する。`delivery = "lossy"`(既定)では満杯時に**最も古い**仕事が
