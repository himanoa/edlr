...
| `apiKey` | string | `""` | INARA の API キー。**平文で保存される**(下記 3 番) |
...
| `uploadHistorical` | boolean | `false` | プラグイン起動より前のイベントも送る(下記 2 番) |
...
### 2. Journal tailer が読み取り位置を永続化しない(二重送信 / 取りこぼし)

**デーモン起動時、tailer は現行 Journal ファイルを先頭から読み直す**
(`core/src/journal/tailer.rs` の `pos = 0` 起点)。つまりデーモンを再起動する
たびに、その日のイベントが丸ごと再配信される。

アップローダにとってこれは二重投稿を意味する。回避として、既定では
**プラグイン起動時刻より古いタイムスタンプのイベントを捨てている**
(`uploadHistorical = false`)。副作用として、**デーモンが止まっていた間に
発生したイベントは送られない**。今のところ「重複を出す」か「取りこぼす」かの
2 択しかない。

**必要なのはプラグイン側の永続化ではなく、tailer 側の位置の永続化**。
`<state-dir>` に「どの Journal ファイルをどこまで読んだか」を保存し、起動時に
そこから再開すればよい:

- 再配信が起きないので、**全プラグインが一律に救われる**(このプラグイン固有の
  問題ではない — イベントを外部へ送る / 集計する類のプラグインは全部同じ穴を踏む)
- デーモンが止まっていた間のイベントも保存位置から拾えるので、上の 2 択が
  そもそも消える

ファイルアクセス capability(`driver-fs`)が入ったので、プラグイン側で
「最後に送ったタイムスタンプ」を持つ回避策も**一応**書けるようになった。
ただしそれはコア側の挙動の後始末をプラグイン作者に押し付ける形で、しかも
プラグインごとに実装が要る。`driver-fs` はプラグイン固有の状態(送信の
バックオフ状況など)に使うべきで、この問題の解決には使わない方がよい。
...
`driver-fs` が入ったので「承認したフォルダに自前で保存する」形は書けるが、
それは平文をユーザーのフォルダへ移すだけで、しかも API キーを持つためだけに
ファイルアクセスの承認を要求することになる(承認画面には「フォルダ内の
ファイルを読み取り・作成・上書き・削除できます」と出る)。**秘密情報の置き場所
としては、素直に `secret` 型を足すのが正しい。**
